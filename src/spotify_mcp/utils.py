...
def parse_playlist(playlist_item: dict, username, detailed=False) -> Optional[dict]:
    ...
    narrowed_item = {
        ...
        'total_tracks': (playlist_item.get('items') or playlist_item.get('tracks') or {}).get('total', 0),
    }
    if detailed:
        narrowed_item['description'] = playlist_item.get('description')
        tracks = []
        for t in (playlist_item.get('items') or playlist_item.get('tracks') or {}).get('items', []):
